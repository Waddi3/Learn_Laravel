...
class CommentsTableSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // إضافة تعليقات لمنشورات المدونة
        Comment::factory($commentsCount)->make()->each(function($comment) use($posts, $users) {
            $comment->commentable_id = $posts->random()->id;
            $comment->commentable_type = BlogPost::class; // استخدم العلاقة polymorphic
            $comment->user_id = $users->random()->id;
            $comment->save();
        });

        // إضافة تعليقات للمستخدمين
        Comment::factory($commentsCount)->make()->each(function($comment) use($users) {
            $comment->commentable_id = $users->random()->id;
            $comment->commentable_type = User::class; // استخدم العلاقة polymorphic
            $comment->user_id = $users->random()->id;
            $comment->save();
        });
    }
